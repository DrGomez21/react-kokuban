...
export function TablerosPage() {
    ...
    const getTableros = async () => {
        ...
        const tablerosEncontrados = response.data.filter(tab => tab.espacio === espacio.id)
    ...
    const handleDeleteTablero = (tabId) => {
        setTableros(prevTabs => prevTabs.filter(tablero => tablero.id !== tabId));
    };
    ...
    const crearTablero = handleSubmit(async data => {
        try {
            ...
            const response = await axios.post('http://localhost:8000/api/tableros/', tableroCreado, {
                headers: {
                    Authorization: `Token ${token}`
                }
            })
        ...
        <div className="bg-[#E7E6EF] h-screen">
            ...
            <div className="px-8">
                <div className="grid grid-cols-3 gap-3 my-3">
                    {
                        tableros.map(tab => (
                            <TableroItem key={tab.id} tablero={tab} espacio={espacio} token={token} usuario={user} onDelete={handleDeleteTablero}/>
                        ))
                    }
            ...
            <Modal isOpen={mostrarModal} onClose={() => setMostrarModal(false)}>
                ...
                <form onSubmit={crearTablero} className="mt-4">
                    <div className="mb-2">
