...
export function Lista({
    listaId, token, tareas, cant_max,
    allTasks, usuario, lista, onEliminarLista, onActualizarTitulo, setActiveCard,
    ...
    useEffect(() => {
        const tareasEnLista = tareas.filter(tarea => tarea.estado === listaId).length;
        ...
        <div className={`bg-white h-auto min-w-64 shadow-[.5rem_.5rem_#121212] border-4 border-[#121212] p-4 w-80 ${isListaLlena ? 'border-red-500' : ''}`}>
            ...
            {isListaLlena && (
                <span className="flex justify-center gap-2 w-full text-center text-red-500 montserrat-semibold mb-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="icon icon-tabler icons-tabler-outline icon-tabler-alert-triangle">
                ...
                <div className='p-2'>
                    ...
                    <div className="space-y-2">
                        ...
                        <form className='justify-center' onSubmit={actrualizarCantMaxima}>
                            <label className="flex-grow montserrat-regular mr-4">Máximo de tarjetas:</label>
                            <input
                                type="number"
                                value={maxTarjetas}
